...
class ShellProduction_30_mm(production.Production):
    
    def __init__(self, D, H, Th, steel, parts, lotus=0):
        self.name = 'Обечайка 30 мм'
        self.D = D
        self.Th = Th
        self.H = H
        self.parts = parts
    ...
    def calc_hours(self):
        ...
        # 9. Сварка стыка 
        self.hours[9] = ['welding', (self.H / 1000) * self.DB.welding_madp_30_mm[self.Th] * 1.5 * 0.55 * 1.2 * 1.2 * 1.2 * 1.2 * 0.8]
        ...
        if self.parts > 1:
            self.hours[16] = ['pravka_many_parts', 1.6]
